...
_PROCEDURAL_PATTERNS = [
    # 役職者の発言（○委員長（氏名）、○会長（氏名）等）
    re.compile(r"^○(委員長|会長|団長|座長|理事|主査)（"),
    # ○xx委員長、○xx主査 等（括弧なし形式）
    re.compile(r"^○.{1,10}(委員長|主査|会長|議長|事務総長|事務次長)"),
    # 開会・散会・休憩
    re.compile(r"(これより.{0,10}(会議|連合審査会)を開きます)"),
    re.compile(r"(散会いたします|休憩いたします|再開いたします)"),
    re.compile(r"(ただいまから.{1,30}を(開会|開議)いたします)"),
    # 議決・採決
    re.compile(r"(御異議なしと認めます|異議なしと認め)"),
    re.compile(r"(これにて.{1,20}(質疑|討論|採決)は終了)"),
    re.compile(r"(本日は、これにて散会)"),
    # 委員異動報告
    re.compile(r"委員の異動について御報告"),
    # 参考人・公述人への定型挨拶
    re.compile(r"(参考人|公述人).{0,10}(一言|御礼|御挨拶)"),
    re.compile(r"以上で(参考人|公述人)に対する質疑は終了"),
    re.compile(r"以上で.{0,10}(説明の聴取|意見の陳述)は終了"),
    # 理事会決定事項報告
    re.compile(r"理事会決定事項について御報告"),
    # 議事進行指示
    re.compile(r"(速記を(止め|起こ)してください)"),
    re.compile(r"(氏名標をお立て|発言を希望される方は)"),
    # 政府参考人（答弁者）
    re.compile(r"^○政府参考人（"),
    # 参考人回答
    re.compile(r"^○参考人（"),
