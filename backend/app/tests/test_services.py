...
class TestRankingService:
    ...
    def test_ranking_filter_chamber(
        self, db: Session, ranking_seed
    ):
    ...
    def test_ranking_sort_by_axis(
        self, db: Session, ranking_seed
    ):
        results = get_ranking(db, sort_by="legislative_activity")
        assert (
            results[0]["score"].legislative_activity
            >= results[1]["score"].legislative_activity
        )

    def test_ranking_limit_offset(
        self, db: Session, ranking_seed
    ):
    ...
    def test_ranking_nonexistent_session(
        self, db: Session, ranking_seed
    ):
    ...
    def test_ranking_filter_role_category(
        self, db: Session, ranking_seed
    ):
        results = get_ranking(
            db, role_category="opposition_senior"
        )
        assert len(results) == 1
...
class TestSendWebhook:
    ...
    def test_sends_embed(self, mock_settings, mock_client_cls):
        ...
        mock_client_cls.return_value.__exit__ = (
...
class TestNotifyFunctions:
    ...
    def test_batch_complete_all_success(self, mock_webhook):
        results = [
            {
                "name": "speeches", "status": "ok",
                "records": 100, "elapsed": 10.0,
            },
            {
                "name": "bills", "status": "ok",
                "records": 50, "elapsed": 5.0,
            },
        ]
    ...
    def test_batch_complete_with_failures(self, mock_webhook):
        results = [
            ...
            {
                "name": "scoring", "status": "error",
                "records": 0, "error": "DB down",
                "elapsed": 1.0,
            },
        ]
